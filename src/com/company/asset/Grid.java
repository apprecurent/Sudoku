...
public class Grid {
    ...
    /**
     * Gets all the taken squares
     *
     * @return List of the taken squares
     */
    ...
    public boolean continuousSolve() {
        ...
        while (startNumber < 10) {
            ...
            while (!isFilled()) {
                ...
                for (int i = start; i < emptySquares.size(); i++) {
                    ...
                    for (int j = 0; j < 9; j++) {
                        ...
                        if (square.isEmpty()) {
                            square.setNumber(startNumber, false);
                        } else {
            ...
            for (Square square : emptySquares) {
                theseValues.add(square.getNumber());
            }
 ...
 */
    ...
    public void solve(boolean locked) {
        ...
        for (Square availableSquare : getAvailableSquares()) {
            Set<Integer> takenNumbersSet = new HashSet<>();
            for (int i = 1; i < 10; i++) {
                takenNumbersSet.add(i);
            }

            for (Square fieldSquare : availableSquare.getAssociatedSquares()) {
                if (!fieldSquare.isEmpty()) {
                    takenNumbersSet.remove(fieldSquare.getNumber());
                }
            }
        ...
        while (!isFilled()) {
            ...
            for (int i = index; i < squareList.size(); ) {
                ...
                if (currentSquareIndex > valueList.size()) {
                    currentSquareIndex = -1;
                }
                for (int j = currentSquareIndex; j < valueList.size(); j++) {
                    if (square.getNumber() == valueList.get(valueList.size() - 1)) {
                        break;
                    }
    ...
    public List<Square> getLockedSquares() {
        List<Square> lockedSquares = new ArrayList<>();
        for (Square square : getSquares()) {
            if (square.isLocked()) {
    ...
    public void selectedSolveV2(int number, boolean lock) {
        ...
        for (Square square : getAvailableSquares()) {
            for (Square innerSquare : square.getAssociatedSquares()) {
                if (innerSquare.getNumber() != 0) {
                    takenNumbers.add(innerSquare.getNumber());
                }
            }
